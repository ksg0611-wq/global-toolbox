...
export default function App() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50 dark:bg-zinc-955 text-gray-900 dark:text-zinc-200 transition-colors duration-300">
      ...
      {/* 5. Footer */}
      <Footer 
        onOpenPrivacy={() => setActiveTool('privacy-policy')}
        onOpenTerms={() => setActiveTool('terms-of-service')}
        onOpenContact={() => setActiveTool('contact')}
      />
      ...
      {activeTool === 'privacy-policy' && (
        <PrivacyPolicy onClose={() => setActiveTool(null)} />
      )}
      {activeTool === 'terms-of-service' && (
        <TermsOfService onClose={() => setActiveTool(null)} />
      )}
